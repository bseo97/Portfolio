...
export default function ProjectDetail() {
  ...
  // GitHub repository button, styled to match AboutMe's "Download Resume" pill
  // (gradient + nested icon circle). Demo button removed per request.
    ...
    <div className="flex flex-wrap gap-3 justify-center mt-7 mb-2">
      {project.repositoryLink ? (
        <a
          ...
          className="group inline-flex items-center gap-3.5 pl-6 pr-2.5 py-2.5 rounded-full text-white font-semibold text-base transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-0.5 active:scale-[0.98]"
          style={{ background: 'linear-gradient(140deg, #7fe4e4 0%, #53c9c9 50%, #37b6b6 100%)', boxShadow: '0 12px 26px -10px rgba(83,201,201,0.7), inset 0 1px 1px rgba(255,255,255,0.4)' }}
        >
          <span>Explore Repository</span>
          <span
            className="w-[34px] h-[34px] rounded-full flex items-center justify-center transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:scale-105"
            style={{ background: 'rgba(255,255,255,0.22)', boxShadow: 'inset 0 1px 1px rgba(255,255,255,0.3)' }}
          >
        ...
        <div className="glass mt-16 rounded-[1.75rem] p-8 md:p-10 transition-all duration-500">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="text-center px-4">
              ...
              <div className="text-[11px] font-semibold uppercase tracking-[0.2em]" style={{ color: isDarkMode ? 'rgba(242,242,242,0.5)' : 'rgba(26,26,26,0.5)' }}>Status</div>
            </div>
          </div>
        </div>
      </div>
